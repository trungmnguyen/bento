...
- Before designing or modifying quantitative trading logic, risk rules, math formulations, or Clean Architecture layers:
  - Query existing institutional memory: `bento memory list`
...
- Never report a feature or fix as complete without running ground-truth contract assertions:
  - Single contract: `bento run <contract.json>`
  - Regression suite: `bento suite <directory>`
- If an agent fix fails assertions, run closed-loop healing:
  - `bento auto --task <task.md> --contract <contract.json> --max-iterations 3`
